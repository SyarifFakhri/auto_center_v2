...
class MasterWindow(QMainWindow):
	...
	def runMachineCalibrationCheck(self):
		...
		msg.exec()
		self.programCam.arduinoController.isCamHolderPneumaticsWorking()
		msg.done(1)

		msg.setText("Check pogo pin pneumatics")
		msg.exec()
		self.programCam.arduinoController.isPogoPinPneumaticsWorking()
		msg.done(1)

		msg.setText("Please keep left button pressed")
		msg.exec()
		self.programCam.arduinoController.checkLeftButtonPress()
		msg.done(1)
	...
	def resetStatistics(self):
		ret = QtWidgets.QMessageBox.question(self, "Warning",
		                                     "Are you sure? This will delete all Statistics for the currently selected camera.",
		                                     QtWidgets.QMessageBox.Yes, QtWidgets.QMessageBox.Abort)
	...
	def programCamera(self):
		...
		if self.bothButtonsEnabled:  # it is only enabled when in the main menu
			...
			if aiEnabled:
				...
				if detected != 'right':
					ret = QtWidgets.QMessageBox.question(self, "Warning",
					                                     "PCB Orientation is detected as " + str(
						                                     detected) + ". Continue?",
					                                     QtWidgets.QMessageBox.Yes, QtWidgets.QMessageBox.Abort)
				else:
	...
	def showSplashWindow(self, param):
		...
		self.runMachineCalibrationCheck() #Not sure if this is needed or not test tomorrow
		...
		self.programCam.arduinoController.connectCallBacks()
	...
	def showStatsMenu(self, event):
		...
		self.statsWindow.init_ui(self, self.database.all()[0][self.programCam.currentCameraType],
		                         self.programCam.currentCameraType)
		...
		# self.showMaximized()
		self.statsWindow.exportCsvButton.clicked.connect(self.exportCSV)
		if debugConfigs.FULLSCREEN:
			self.showFullScreen()
		else:
			self.showMaximized()
	...
	def exportCSV(self):
		cp1pData = self.database.all()[0]['cp1p']
		d55lData = self.database.all()[0]['d55l']

		with open('cp1p_data.csv', mode='w') as csvFile:
			csvWriter = csv.writer(csvFile, delimeter = ',', quotechar='"', quoting=csv.QUOTE_MINIMAL)
			for data in cp1pData:
				csvWriter.writerow(data)
		with open('d55l_data.csv', mode='w') as csvFile:
			csvWriter = csv.writer(csvFile, delimeter=',', quotechar='"', quoting=csv.QUOTE_MINIMAL)
	...
	def saveSettings(self):
		...
		self.programCam.reloadDetector(currentCamera)
